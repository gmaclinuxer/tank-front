    ...
    methods: {
      ...
      triggerUpload() {
        ...
        if (that.user.size_limit >= 0) {
          if (matter.file.size > that.user.size_limit) {
            that.$message.error(this.$t('matter.sizeExceedLimit', humanFileSize(matter.file.size), humanFileSize(that.user.size_limit)))
            return;
          }
        }
